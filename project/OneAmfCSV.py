import pyshark
import csv

# Input and output file paths
pcap_file = 'project/AMF_1_1_A.cap'
csv_file = 'kotstput_packets.csv'

# Open the pcap file
cap = pyshark.FileCapture(pcap_file, display_filter='ngap')

# Define CSV headers
headers = [
    'Time', 'Length', 'Protocol', 'Source IP', 'Destination IP',
    'initiatingmessage_element', 'procedurecode',
    'nas_pdu', 'nas_5gs_security_header_type', 'nas_5gs_mm_message_type'
]

# Open CSV file for writing
with open(csv_file, 'w', newline='', encoding='utf-8') as f:
    writer = csv.DictWriter(f, fieldnames=headers, delimiter=';')
    writer.writeheader()

    for pkt in cap:
        try:
            row = {
                'Time': pkt.sniff_time,
                'Length': pkt.length,
                'Protocol': pkt.highest_layer,
                'Source IP': pkt.ip.src if hasattr(pkt, 'ip') else '',
                'Destination IP': pkt.ip.dst if hasattr(pkt, 'ip') else '',
                'initiatingmessage_element': '',
                'procedurecode': '',
                'nas_pdu': '',
                'nas_5gs_security_header_type': '',
                'nas_5gs_mm_message_type': ''
            }

            # Extract NGAP fields
            if hasattr(pkt, 'ngap'):
                for field in row.keys():
                    if hasattr(pkt.ngap, field):
                        row[field] = getattr(pkt.ngap, field)

            writer.writerow(row)

        except Exception as e:
            print(f"Error processing packet: {e}")

# Close the capture
cap.close()

print(f"\nCSV file '{csv_file}' written successfully.")
